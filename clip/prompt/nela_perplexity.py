...
WRONG (single utterance, no context): Utterance #5 only → incomplete thought
CORRECT (merged sequence): Utterance #3 (setup) + #4 (build) + #5 (climax) + #6 (resolution) → Forms complete RAMP→PEAK→VALLEY arc
...
5. GENERATE two passes (try not to collapse halfway):
   ...
   - **Pass 2**: refine top candidates (ranked) → improve titles, hook_descriptions, summaries
6. Choose the final set of clips — number of clips is your decision, but always at least one. 
   If you can't do that, maybe power off.
...
- You must return **at least one clip**.
